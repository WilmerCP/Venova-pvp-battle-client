...
export default function PokeSelection({ onSelectPokemon, onBack, availablePokemon, forced }) {
    ...
    const { plainBackground, contrastText } = theme;

    return (
        <div className={`flex flex-row h-full gap-2 p-2 ${plainBackground}`}>
            <div className="flex flex-col items-center justify-center gap-2 w-24 shrink-0">
                <span className={`text-xs font-semibold text-center leading-tight ${contrastText}`}>
                ...
                {!forced && (
                    <button
                        ...
                        className="flex items-center justify-center w-10 h-10 rounded-sm
                              bg-red-600 hover:bg-red-400 active:translate-y-0.5
